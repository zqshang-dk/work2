...
SolveResult pntpos_multi(const vector<SatData>& obs) {
    SolveResult res;
    res.success = false;
    res.Parameter = VectorXd::Zero(6);          //// X,Y,Z, dtG, dtC, dtR
    ...
    for (int i = 0; i < n;i++){
        ...
        double dt_sys = (sys=='G'?dt_GPS:(sys=='C'?dt_BDS:dt_GLO));
        double P_cal = dist + dt_sys;
        V(i) = P_cal - P_obs;
    }
    //计算验后单位权方差
    double vtwv = V.transpose() * W * V;
    double sigma0 = sqrt(vtwv / (n - 6));

    res.Parameter << Xr, Yr, Zr, dt_GPS, dt_BDS,  dt_GLO;
    res.sigma0 = sigma0;
...
int main() {
    ...
    while (getline(infile, line)) {
        ...
        if (line[0] == '#') {
            ...
            // 进行定位解算
            if (epoch_obs.size() >= 6) {
                vector<double> result(6, 0.0); // 存放解 [x, y, z, dt(4)]
